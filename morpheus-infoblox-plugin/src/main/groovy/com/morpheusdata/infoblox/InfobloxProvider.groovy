...
class InfobloxProvider implements IPAMProvider, DNSProvider {
			...
			if(integration) {
				...
				switch(recordType) {
					case 'A':
						...
						results = RestApiUtil.callApi(serviceUrl, apiPath, poolServer.serviceUsername, poolServer.servicePassword, new RestApiUtil.RestOptions(headers:['Content-Type':'application/json'], ignoreSSL: poolServer.ignoreSsl,body:body, cookies: opts.cookies, httpClient: opts.httpClient, keepAlive: opts.keepAlive), 'POST')
		...
		try {
			if(integration) {
				morpheus.network.getPoolServerByAccountIntegration(integration).doOnSuccess({ poolServer ->
					...
					def results = RestApiUtil.callApi(serviceUrl, apiPath, poolServer.serviceUsername, poolServer.servicePassword, new RestApiUtil.RestOptions(headers:['Content-Type':'application/json'], ignoreSSL: poolServer.ignoreSsl,
		...
		try {
			...
			if(testResults.success) {
				...
				if(testResults.getCookie('ibapauth')) {
					RestApiUtil.callApi(poolServer.serviceUrl, getServicePath(poolServer.serviceUrl) + 'logout', poolServer.serviceUsername, poolServer.servicePassword, new RestApiUtil.RestOptions([headers:['Content-Type':'application/json'], ignoreSSL: poolServer.ignoreSsl] + addOnMap), 'POST')
				}
				if(addOnMap?.reuse) {
					RestApiUtil.shutdownClient(addOnMap)
				}
	...
	ServiceResponse listZoneRecords(NetworkPoolServer poolServer, String zoneName, String recordType = 'record:a', Map opts=[:]) {
		...
		while(hasMore && attempt < 1000) {
			...
			//load results
			def results = RestApiUtil.callApi(serviceUrl, apiPath, poolServer.serviceUsername, poolServer.servicePassword, new RestApiUtil.RestOptions(headers:['Content-Type':'application/json'],
	...
	ServiceResponse<NetworkPoolIp> createHostRecord(NetworkPoolServer poolServer, NetworkPool networkPool, NetworkPoolIp networkPoolIp, NetworkDomain domain = null, Boolean createARecord = false, Boolean createPtrRecord = false) {
		...
		if(results.success) {
			...
			if(createARecord && domain) {
				...
				results = RestApiUtil.callApi(serviceUrl, apiPath, poolServer.serviceUsername, poolServer.servicePassword, new RestApiUtil.RestOptions(headers:['Content-Type':'application/json'], ignoreSSL: poolServer.ignoreSsl,
		...
		if(poolIp.externalId) {
			...
			if(results?.success && !results.hasErrors()) {
				if(poolIp.internalId) {
					...
					//we have an A Record to delete
					results = RestApiUtil.callApi(serviceUrl, apiPath, poolServer.serviceUsername, poolServer.servicePassword, new RestApiUtil.RestOptions(headers:['Content-Type':'application/json'], ignoreSSL: poolServer.ignoreSsl), 'DELETE')
				}
				if(poolIp.ptrId) {
...
//				attrOpts += standardProvisionService.getContainerScriptConfigMap(opts.container,'preprovision')
		...
		if(results?.success && !results?.hasErrors()) {
			...
			results = RestApiUtil.callApi(serviceUrl, networkPath, poolServer.serviceUsername, poolServer.servicePassword, new RestApiUtil.RestOptions(headers:['Content-Type':'application/json'], ignoreSSL: poolServer.ignoreSsl,
	...
	private ServiceResponse listNetworks(NetworkPoolServer poolServer, Map opts) {
		...
		if(doPaging == true) {
			...
			while(hasMore && attempt < 1000) {
				...
				//load results
				def results = RestApiUtil.callApi(serviceUrl, apiPath, poolServer.serviceUsername, poolServer.servicePassword, new RestApiUtil.RestOptions(headers:['Content-Type':'application/json'], queryParams: pageQuery,
			...
			def results = RestApiUtil.callApi(serviceUrl, apiPath, poolServer.serviceUsername, poolServer.servicePassword, new RestApiUtil.RestOptions(headers:['Content-Type':'application/json'], ignoreSSL: poolServer.ignoreSsl, queryParams:pageQuery,
