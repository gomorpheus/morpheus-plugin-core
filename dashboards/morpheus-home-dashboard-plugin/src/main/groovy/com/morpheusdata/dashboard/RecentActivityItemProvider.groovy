 ...
 */
@Slf4j
class RecentActivityItemProvider extends AbstractDashboardItemTypeProvider {

	Plugin plugin
	MorpheusContext morpheusContext

	RecentActivityItemProvider(Plugin plugin, MorpheusContext context) {
		this.plugin = plugin
	...
	@Override
	String getCode() {
		return 'dashboard-item-recent-activity'
	}
	...
	DashboardItemType getDashboardItemType() {
		...
		rtn.code = getCode()
		rtn.category = 'health'
		rtn.title = 'recent activity'
		rtn.description = 'recent activity'
		rtn.uiSize = 'lg'
